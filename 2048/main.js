...
var score = 0;
var has_conflicated = new Array();     //解决连续消除的标记
            ...
            if(board[i][j]!=0){
                for(var k=0;k<j;k++){
                    if(board[i][k]==0&&no_block_horizontal(i,k,j,board)){
                        ...
                        board[i][j]=0;
                        break;
                    }else if(board[i][k]==board[i][j]&&no_block_horizontal(i,k,j,board)&& !has_conflicated[i][k]){
                        show_move_animation(i,j,i,k);
                        board[i][k]+=board[i][j];
                        board[i][j]=0;
                        score+=board[i][k];
                        update_score(score);
                        has_conflicated[i][k]=true;
                        break;
                    }
                }
            }
